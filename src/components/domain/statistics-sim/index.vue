        ...
        <div class="mt-4 text-center text-xs overflow-auto lg:overflow-visible">
            <div class="py-2 border-y-[1px] border-[#EDF2F7]">
                <div class="flex gap-2 py-5">
                    ...
                    <HeaderText  class="min-w-[230px] w-1/5">
                        Номер SIM
                    </HeaderText>
                    <HeaderText  class="min-w-[192px] w-[15%]">
                        Данные паспорта
                    </HeaderText>
                    <HeaderText  class="min-w-[120px] w-[10%]">
                        Цена
                    </HeaderText>
                    <HeaderText class="min-w-[192px] w-[15%]">
                        Статус
                ...
                <div class="flex items-center w-full gap-2">
                    ...
                    @input="filterId">
                    <input type="text" class="input min-w-[192px] w-[15%]" v-model="filtr.phoneNumber" :maxlength="maxlengthPhone(filtr.phoneNumber)"
                    @input="filterPhoneNumber">
                    <input type="text" class="input min-w-[230px] w-1/5" v-model="filtr.imsi"
                    @input="filterImsi">
                    <input type="text" class="input min-w-[72px] w-[5%]" v-model="filtr.passportSerial"
                    maxlength="2" @input="filterPassportSerial">
                    <input type="text" class="input min-w-[120px] w-[10%]" v-model="filtr.passportNumber"
                    maxlength="7" @input="filterPassportNumber">
                    <input type="text" class="input min-w-[112px] w-[10%]" v-model="filtr.price"
                    @input="filterPrice">
                    <select @change="((e) => filtr.statusId = e.target.value)" class="input min-w-[192px] w-[15%]">
                        <option selected/>
                        <option v-for="item in statuses" :key="item.id" :value="item.id">{{item.name}}</option>
                    </select>
                    <p class="text-[10px] input pt-0.5 min-w-[176px] w-[15%] bg-white cursor-pointer overflow-clip"
                    @click="changeDate">
                        {{calendarText}}
                    </p>
                    <div class="min-w-[80px] w-[5%]">
            ...
            <div v-for="item in statistics.content" :key="item.id" class="flex gap-2 border-b-[1px] border-[#EDF2F7] py-[30px]">
                ...
                <Text  class="min-w-[230px] w-1/5">
                    {{item.imsi}}
                </Text>
                <Text  class="min-w-[192px] w-[15%]">
                    {{item.passportSerial}}{{item.passportNumber}}
                </Text>
                <Text  class="min-w-[120px] w-[10%]">
                    {{item.price}}
                </Text>
                <Text class="min-w-[192px] w-[15%]">
...
export default {
    ...
    methods: {
        ...
        async get_statistics() {
            let payload = {
                request: `change-sim?page=${this.page}&size=${this.activePaginationList}`,
                key: 'statistics',
                body: []
            }
        ...
        filterImsi() {
            this.filtr.imsi = clearSymbols(this.filtr.imsi)
        },
        ...
        async changePagination(e) {
            ...
            let payload = {
                request: `change-sim?page=${this.page}&size=${this.activePaginationList}&search=${this.filtr.id.length > 0 ? `id:${this.filtr.id},` : ''}${this.filtr.phoneNumber.length > 0 ? `phoneNumber:*${this.filtr.phoneNumber}*,` : ''}${this.filtr.imsi.length > 0 ? `imsi:*${this.filtr.imsi}*,` : ''}${this.filtr.passportSerial.length > 0 ? `passportSerial:*${this.filtr.passportSerial}*,` : ''}${this.filtr.passportNumber.length > 0 ? `passportNumber:*${this.filtr.passportNumber}*,` : ''}${this.filtr.price.length > 0 ? `price:${this.filtr.price},` : ''}${this.filtr.statusId.length > 0 ? `statusId:${this.filtr.statusId},` : ''}${this.filtr.createdAt[0].length > 0 ? `createdAt!${dateFrom},` : ''}${this.filtr.createdAt[1].length > 0 ? `createdAt@${dateTo}` : ''}`,
                key: 'statistics',
                body: []
            }
            await this.get_page(payload)
        ...
        async exportExcel() {
                ...
                let exportRequest= {
                    request: `change-sim/export?search=${this.filtr.id.length > 0 ? `id:${this.filtr.id},` : ''}${this.filtr.phoneNumber.length > 0 ? `phoneNumber:*${this.filtr.phoneNumber}*,` : ''}${this.filtr.imsi.length > 0 ? `imsi:*${this.filtr.imsi}*,` : ''}${this.filtr.passportSerial.length > 0 ? `passportSerial:*${this.filtr.passportSerial}*,` : ''}${this.filtr.passportNumber.length > 0 ? `passportNumber:*${this.filtr.passportNumber}*,` : ''}${this.filtr.price.length > 0 ? `price:${this.filtr.price},` : ''}${this.filtr.statusId.length > 0 ? `statusId:${this.filtr.statusId},` : ''}${this.filtr.createdAt[0].length > 0 ? `createdAt!${dateFrom},` : ''}${this.filtr.createdAt[1].length > 0 ? `createdAt@${dateTo}` : ''}`,
                    key: 'exportFileId',
                    body: null
                }
            ...
            async downloadExcel() {
                ...
                test.setAttribute('download', 'statistics-sim.xlsx');
